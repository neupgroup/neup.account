...
function BackButtonInner({ backsTo, href, className, navigationTargets }: BackButtonProps) {
  ...
  return (
    <a href={targetHref} className={cn('flex items-center gap-2 text-sm font-bold text-foreground hover:underline underline-offset-4', className)} onClick={(event) => { event.preventDefault(); window.location.assign(targetHref); }}>
...
export function BackButton(props: BackButtonProps) {
  return (
    <Suspense fallback={<a href={props.backsTo ?? props.href ?? '/account'} className={cn('flex items-center gap-2 text-sm font-bold text-foreground hover:underline underline-offset-4', props.className)}><ChevronLeft className="h-4 w-4" />Go back</a>}>
