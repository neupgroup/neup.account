...
- [ ] Fix the current type-check baseline errors reported by `npx tsc --noEmit --pretty false` after the `inapp/permissions` move: restore or replace `getAccessableAccountsWithPermissions` in `app/bridge/api.v1/accounts/route.ts`, resolve Prisma type drift for `errorLog` and `Asset.name` in `core/helpers/logger.ts` / `core/helpers/metadata.ts`, and fix stale `@/core/context/ProfileContext` import in `core/hooks/use-page-title.ts`.
