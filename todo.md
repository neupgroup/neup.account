...
- [ ] Remove stale `application.basics.edit` references from `prisma/scripts/runner-neupaccount.ts`; the live `neup.account` `authz_permission` row was deleted, but the old runner can reinsert it during a rebuild.
...
- [ ] Fix remaining core boundary violations: `core/helpers/link.ts` and `core/helpers/navigation.ts` import `app/(manage)/application/_lib/application-mode`, and `core/providers/session.tsx` imports account services. Move app-specific mode/session behavior out of `core` or pass plain values into generic core helpers.
- [ ] Reconcile logger placement after moving it to `logica/logger/files`: current app and service callers import `@/logica/logger/files`, which conflicts with the documented app/services/logica dependency direction.
