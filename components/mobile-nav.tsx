    ...
    const navConfig: NavSection[] | null = useMemo(() => {
        ...
        if (isManaging) {
            const managedItems = [
                ...
                { href: "/accounts/branches", label: "Branches", description: "Manage brand branches.", icon: iconMap['LinkedAccounts'], requiredPermissions: ['linked_accounts.brand.manage.self'] },
            ].filter((item) => hasAnyPermission(permissions, item.requiredPermissions));
